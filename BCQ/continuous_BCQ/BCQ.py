...
class Critic(nn.Module):
	...
	def q1(self, state, action):    # what this meaning?
		q1 = F.relu(self.l1(torch.cat([state, action], 1)))
		q1 = F.relu(self.l2(q1))
		q1 = self.l3(q1)
		return q1
...
class BCQ(object):
	def __init__(self, state_dim, action_dim, max_action, device, discount=0.99, tau=0.005, lmbda=0.75, phi=0.05, lr=1e-3):
	...
	def train(self, replay_buffer, iterations, batch_size=100):
		...
		for it in range(iterations):
			...
			KL_loss	= -0.5 * (1 + torch.log(std.pow(2)) - mean.pow(2) - std.pow(2)).mean()  # VAE KL散度
			vae_loss = recon_loss + 0.5 * KL_loss
			vae_loss_seq.append(vae_loss)
			...
			with torch.no_grad():
				...
				target_Q = reward + not_done * self.discount * target_Q    # TODO: not_done and discount 作用？
